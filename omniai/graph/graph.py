...
class Graph:
    ...
    def add_conditional_edges(
        ...
        path_map_size = len(path_map) if path_map else 0
        logger.debug(f"⚡ Conditional edge added | source={source} | path_map_size={path_map_size}")
...
class CompiledGraph:
    ...
    async def ainvoke(self, state: State | dict[str, Any]) -> State:
        ...
        logger.info(f"🚀 Graph started | entry={current} | msgs={len(state.messages)}")
        iteration = 0

        try:
            for iteration in range(self.max_iterations):
                if current == END:
                    logger.info(f"✅ Graph execution completed | iterations={iteration} | "
            ...
            if current == END:
                logger.info(f"✅ Graph execution completed | iterations={self.max_iterations} | "
                            f"messages={len(state.messages)}")
                return state
            ...
            logger.error(
                f"❌ Graph execution failed after {iteration} iterations: {exc_name}: {exc}"
            )
            raise
    ...
    def as_handler(self) -> Callable[[Any], Awaitable[Any]]:
        ...
        async def handler(message: OmniMessage) -> OmniMessage:
            ...
            logger.debug(f"📬 Handler invoked | session={session_id} | content_len={content_len}")
            try:
                ...
                logger.debug(f"✅ Reply | len={len(reply.content)} | tools={len(reply.tool_calls)}")
                return reply
            except Exception as exc:
